...
### Decision 6.14 — Defer Xero transactions whose Xero Contact has no AccountNumber
- **What**: `ULBC_XeroIncomeImporter` now skips any transaction where the Xero Contact's `AccountNumber` is blank. No Opportunity is created, no notification fires. The transaction is *deferred* — counted on the sync log via the new field `ULBC_Deferred_No_AccountNumber__c`, and the import watermark is held back so the next nightly run re-pulls it once Martin sets the AccountNumber on the Xero side.
- **Why**: Martin's Xero workflow assigns AccountNumbers in fortnightly batches, not in real time. Before this change every untriaged Xero Contact produced a noise orphan in Salesforce on the day of the bank deposit. 4 of 31 imports on the 2026-05-01 run were noise of this kind (and 9 across the cumulative 2026-04-29 → 2026-05-01 window). The new rule moves the orphan signal to "AccountNumber present but no Salesforce Contact has the matching Trust ID" — i.e. the only orphans Martin sees are the ones that actually need *Salesforce-side* action. The Xero-side queue is its own backlog and doesn't need to be mirrored.
- **Watermark approach**: Per page, track `minDeferredUpdated` (smallest `UpdatedDateUTC` among deferred txns) alongside `maxUpdated`. The new effective watermark is `(minDeferredUpdated <= maxUpdated) ? minDeferredUpdated.addSeconds(-1) : maxUpdated`. Imported txns advance the cursor freely; deferred txns hold it back so Xero's next `If-Modified-Since` query re-includes them. The 1-second offset (not 1 ms) matters because `If-Modified-Since` is RFC 1123 GMT with second precision — a sub-second offset would format identically and exclude the deferred txn from the next response. Idempotency via the existing Unique-External-Id on `ULBC_Xero_Transaction_ID__c` prevents already-imported txns from double-creating on the re-pull.
- **Multi-page note**: with chained pages, a later page's `maxUpdated` could in principle advance past an earlier page's `minDeferred`. In practice the daily volume (~30 txns/run, well under PAGE_SIZE=100) keeps every run on a single page; the multi-page case is rare and recoverable. Not worth threading state through the queueable chain at current volumes — revisit if backfills become routine.
- **Amendment to Decision 6.13**: "Orphan" now means *AccountNumber present but no Salesforce Contact has the matching Trust ID*. Notifications (bell + email) fire for orphans only — never for deferred txns. Email and bell volume drops accordingly.
- **Backfill (this session)**: 9 pre-existing noise orphans (created 2026-04-29 / 04-30 / 05-01, all with `AccountNumber_Seen = null`, total income value £11,139.69) were deleted from production via Anonymous Apex script. Donor-tier engine impact: zero — they had no Primary Contact, so the engine never aggregated them. They will re-import cleanly via the next 02:00 UTC run after Martin sets AccountNumbers in Xero.
- **Permission set incidentally addressed**: while preparing the backfill, discovered that `ULBC_Full_Access` had never been granted FLS on `Opportunity.ULBC_Source__c`, `Opportunity.ULBC_Xero_Transaction_ID__c`, `Opportunity.ULBC_Xero_AccountNumber_Seen__c`, the entire `ULBC_Xero_Sync_Log__c` object, or its non-required fields. The deployed Apex worked because Apex SOQL/DML bypasses FLS by default, but Adrian (System Administrator) couldn't query the fields via the Data API or Anonymous Apex. Added in this commit. Required fields (`ULBC_Run_Started__c`, `ULBC_Status__c`) are auto-granted by Salesforce and cannot appear in `<fieldPermissions>`.
- **Implementation**: 1 modified Apex class + 1 modified Apex test class + 1 new field on `ULBC_Xero_Sync_Log__c` + 1 modified permission set. Tests: 12 → 15 (replaced one, added three). Org-wide tests: 233 → 236 passing, 0 failing. Deployment 2026-05-01 (Deploy ID `0AfSk000001AbKHKA0`).
- **Date**: 2026-05-01
- **Relationship to OQ-053**: this change incidentally suppresses double-counting for the subset of Stripe payouts that happen to arrive in Xero with no AccountNumber, but it is **not** a substitute for OQ-053's proper fix (Stripe Payment Intent dedup). OQ-053 remains open and still gates Stripe production go-live.
